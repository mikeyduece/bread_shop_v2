...
RSpec.describe 'User Recipes API' do
  ...
  context 'user recipes' do
    ...
    it 'returns recipe with ingredients and total percentage' do
      ...
      expect(attributes(:formatted_ingredients)[:ingredients].length).to eq(7)
    end
    ...
    it 'cannot create a recipe with same name as one that already exists' do
      ...
      allow(Recipes::Create).to receive(:call).with(user: user,
                                                    params: params).and_return(double(
                                                                                 'Success', success?: false, errors: errors
                                                                               ))
