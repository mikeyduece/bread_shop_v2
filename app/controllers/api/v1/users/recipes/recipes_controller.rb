      ...
      module Recipes
        class RecipesController < UsersBaseController
          ...
          def index
            ...
            success_response(serialized_recipe(recipes,
                                               meta: { total: recipes.count, per_page: limit, page: offset + 1 }))
          end
          ...
          private def serialized_recipe(recipe, options = {})
            serialized_resource(recipe, RecipeSerializer, options.merge!(include: %i[tags]))
          end

          private def set_user_recipe
            @set_user_recipe ||= current_user.recipes.find_by(id: params[:recipe_id] || params[:id])
          end
